...
`overrides` / `resolutions` pin `undici` to **6.28.1** (Node-20-safe) so Publish and local Node 20 builds stay compatible. Do not bump to undici 7/8 until Publish/CI standardize on a Node that provides `markAsUncloneable` (Node 22+).
