...
const COPY: Record<NonNullable<Props["decision"]>, { preview: string; body: React.ReactNode }> = {
  verified: {
    preview: "You're verified — your PatternProof shares are open.",
    body: (
      <Text style={p}>
        Your bar standing is verified. Any client share waiting on this — including one you already
        accepted — is open now.
      </Text>
    ),
  },
  declined: {
    ...
    body: (
      <Text style={p}>
        We reviewed your attorney application and can't verify it right now. You can reapply after 90
        days. Any client share you were sent stays recorded but will not open until a future
  ...
  suspended: {
    ...
    body: (
      <Text style={p}>
        Your attorney access is suspended, effective now — this covers every case you and anyone on
        your team could reach through it. Write back if you believe this is a mistake.
      </Text>
    ),
  },
  reinstated: {
        ...
        <Container style={container}>
          ...
          {(decision === "verified" || decision === "reinstated") && (
            <Section style={{ margin: "24px 0" }}>
              <Link href="https://pattern-proof.tech/clients" style={button}>
                Open your clients
              </Link>
            </Section>
          )}
...
export const template = {
  component: Email,
  subject: ({ decision }: Record<string, unknown>) => {
    switch (decision) {
      case "declined":
        return "Update on your PatternProof attorney application";
      case "suspended":
        return "Your PatternProof attorney access is suspended";
      case "reinstated":
        return "Your PatternProof attorney access is restored";
      default:
        return "You're verified — PatternProof attorney access is open";
    }
  },
