  ...
  .handler(async ({ data, context }) => {
    ...
    const [inc, ev, vn, cd] = await Promise.all([
      sb
        ...
        .is("deleted_at", null)
        .or(
          `description.ilike.${like},location.ilike.${like},witnesses.ilike.${like},emotional_impact.ilike.${like}`,
        )
      ...
      sb
        ...
        .eq("user_id", uid)
        .or(
          `court_name.ilike.${like},hearing_type.ilike.${like},location.ilike.${like},notes.ilike.${like}`,
        )
    ...
    for (const r of (inc.data ?? []) as Array<{
      id: string;
      date: string;
      description: string;
    ...
    for (const r of (ev.data ?? []) as Array<{
      id: string;
      title: string;
      date: string;
    ...
    for (const r of (cd.data ?? []) as Array<{
      id: string;
      court_name: string;
      hearing_type: string;
      hearing_at: string;
