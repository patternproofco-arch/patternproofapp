...
/**
 * Court-packet PDF builder, extracted so the real build runs in tests.
 *
 * The database client is passed in; every read is scoped to the requesting
 * account and to the ids the account attached to this case. Nothing else can
 * enter the document — AI proposals live in a different table and are never
 * read here, and deleted records are filtered out.
 *
 * Original notes: Court-Packet PDF export. Intentionally de-branded:
...
export type PacketResult = {
  bytes: Uint8Array;
  filename: string;
  counts: { incidents: number; evidence: number; legal: number; threads: number; exhibits: number };
};
...
export async function buildCasePacket(
  ...
  const data = { case_id: args.caseId };
  ...
  return {
    bytes,
    filename: `${slug}-court-packet.pdf`,
    counts: {
