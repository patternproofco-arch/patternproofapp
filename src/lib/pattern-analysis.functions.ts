...
const TOOL_SCHEMA = {
  ...
  properties: {
    pattern_summary: {
      type: "string",
      description:
        "2-4 sentence plain-language summary of the overall pattern, suitable to drop into a court packet. First-person about the survivor's experience, factual, no legal conclusions.",
    },
    escalation_arc: {
      type: "string",
      description:
        "Brief narrative describing whether and how severity has shifted over time. Use phrases like 'increased frequency in spring', not statistics.",
    },
    frequency_trends: {
      type: "array",
      items: {
    ...
    abuse_type_breakdown: {
      ...
      items: {
        ...
        required: ["type", "count", "percent"],
      },
    },
    severity_trajectory: {
      type: "string",
      enum: ["decreasing", "stable", "increasing", "volatile", "unknown"],
    },
    gaps: {
      type: "array",
      items: {
        type: "object",
        properties: {
          gap: { type: "string", description: "What's missing or unclear in the record" },
          suggestion: { type: "string", description: "Gentle, specific suggestion of what to add" },
        },
    ...
    suggested_followups: {
      type: "array",
      items: { type: "string" },
      description: "Concrete next documentation steps in the user's voice",
    },
    corroborating_incident_count: {
      type: "integer",
      description:
        "Plain count of incidents in the provided records that describe the primary pattern. A count only — do not interpret it, rate it, or convert it into a strength or confidence judgement.",
    },
    evidence_list: {
      type: "array",
      items: {
    ...
    pattern_timeline_text: {
      type: "string",
      description:
        "Left-to-right text representation of the cycle using arrows, e.g. 'Calm → Boundary Set → Accusation → Threat → Silence → Charm → Calm → Repeat'.",
    },
      ...
      items: {
        ...
        properties: {
          label: {
            type: "string",
            description:
              "Short factual label of the documented behavior (e.g. 'Non-fatal strangulation described', 'Verbal threat to kill', 'Threat involving a weapon'). Do not use words like 'risk', 'tier', 'high', 'detected'.",
          },
          note: {
            type: "string",
            description:
              "One calm sentence describing what the record shows, hedged. Include the phrase 'documented in your records, not a prediction or clinical assessment' or an equivalent hedge.",
          },
          source_incident_ids: {
            type: "array",
            items: { type: "string" },
            minItems: 1,
            description:
              "Incident id(s) from the input list this item is drawn from. Required — no item without a source citation.",
          },
        },
  ...
  .handler(async ({ data, context }) => {
    ...
    const [incidentsRes, commsRes, flagsRes, cachedRes] = await Promise.all([
      supabase
        .from("incidents")
        .select(
          "id,date,time,location,description,abuse_types,witnesses,emotional_impact,severity_level,source,confirmed_at",
        )
        .eq("user_id", userId)
        .is("deleted_at", null)
      ...
      supabase
        ...
        .select("flag_type,severity_tier,details,created_at")
        .eq("user_id", userId)
    ...
    const incidents = rawIncidents.filter((i) => !(i.source === "ai_extracted" && !i.confirmed_at));
    ...
    if (!data.force && cached) {
      ...
      if (ageHours < 24 && countDelta < 3) {
        return {
          ok: true as const,
          id: cached.id,
          analysis: cached.analysis as unknown as PatternAnalysisResult,
          reviewed_status: (cached.reviewed_status ?? {}) as Record<string, ClaimReviewState>,
          cached: true,
        };
      }
    }
        ...
        tools: [
          {
            type: "function",
            function: {
              name: "record_pattern_analysis",
              description: "Record the pattern analysis",
