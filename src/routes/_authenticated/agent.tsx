  ...
  const Sidebar = (
    ...
    >
      <div className="flex items-center gap-2 px-1">
        <BrandMark size={26} />
        <div className="font-semibold text-[15px]" style={{ color: "var(--pp-ink)" }}>
          PatternProof
        </div>
      </div>
      <button
      ...
      <div
        className="text-[10px] uppercase tracking-[0.12em] font-semibold px-2 pt-2"
      ...
      <nav className="flex-1 overflow-y-auto -mx-1 min-h-[80px]">
        {!loaded && (
          <div className="px-2 py-2 text-sm" style={{ color: "var(--pp-muted)" }}>
            Loading…
          </div>
        )}
        {loaded && threads.length === 0 && (
          <div className="px-2 py-2 text-sm leading-relaxed" style={{ color: "var(--pp-muted)" }}>
        ...
        {threads.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className="group flex items-center gap-1 mx-1 rounded-[16px] transition-shadow"
              style={{ boxShadow: active ? "var(--pp-shadow-in-sm)" : "none" }}
            >
              <button
                onClick={() => {
                  setMobileOpen(false);
                  navigate({ to: "/agent/$threadId", params: { threadId: t.id } });
                }}
                className="flex-1 text-left px-2 py-2 text-sm truncate flex items-center gap-2 rounded-[16px]"
      ...
      <div
        className="text-[10px] uppercase tracking-[0.12em] font-semibold px-2 pt-1"
        style={{ color: "var(--pp-muted)" }}
      >
        Workspace
      </div>
        ...
        {NAV_LINKS.map(({ to, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className="flex items-center gap-2.5 mx-1 px-2 py-2 rounded-[16px] text-sm transition-shadow"
              style={{
                color: "var(--pp-ink)",
                boxShadow: active ? "var(--pp-shadow-in-sm)" : "none",
      ...
      <p className="text-[11px] px-2 pt-1 leading-relaxed" style={{ color: "var(--pp-muted)" }}>
        Information only — not legal advice. If you're in immediate danger, call 911 or the National
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="md:hidden fixed top-3 left-3 z-40 p-2"
        style={{
          background: "var(--pp-card)",
          borderRadius: 16,
          boxShadow: "var(--pp-shadow-sm)",
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <div
            className="relative w-[88%] max-w-[320px] h-full p-3"
            style={{ background: "var(--background)" }}
          >
            <button
              onClick={() => setMobileOpen(false)}
