...
function EvidencePage() {
  ...
  const runReview = useCallback(async (row: EvidenceRow, mime?: string) => {
    setReviewFor(row);
    setDraft(null);
    setReviewError(null);
    setReviewBusy(true);
    try {
      ...
      const guessedMime = mime
        ?? (row.file_type === "image" ? "image/jpeg"
          : row.file_type === "document" ? "application/pdf"
          : "");
      if (!guessedMime) {
        setReviewError("Only images and PDFs can be reviewed by AI right now.");
        return;
      }
      const res = await extractFn({ data: { signedUrl: signed.data.signedUrl, mimeType: guessedMime } });
      if (!res.ok) {
        setReviewError(
  ...
  const closeReview = () => {
    setReviewFor(null); setDraft(null); setReviewError(null); setReviewBusy(false);
  };
  ...
  const saveDraftAsIncident = async () => {
    ...
    const ins = await supabase.from("incidents").insert({
      user_id: user.id,
              ...
              return (
                <div key={it.id} className="card-pp">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    {(it.file_type === "image" || it.file_type === "document") && !it.linked_incident_id && (
                      <button onClick={() => runReview(it)} className="btn-ghost inline-flex items-center gap-1 text-[12px]">
          ...
          >
            <div className="mb-2 flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 font-serif text-[22px] leading-tight">Review draft incident</div>
                <p className="mt-1 text-[13px]" style={{ color: "var(--muted-foreground)" }}>
                  This draft was read from <span style={{ fontWeight: 600 }}>{reviewFor.title}</span>. Edit anything before saving — nothing is shared until you do.
                </p>
              </div>
              <button onClick={closeReview} className="btn-ghost" style={{ padding: 6 }}>✕</button>
            </div>

            {reviewBusy && (
              <div className="my-8 text-center text-[14px]" style={{ color: "var(--muted-foreground)" }}>
                Reading the file…
              </div>
            )}
            ...
            {!reviewBusy && draft && (
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <Field label="Date">
                  <input type="date" className="input-pp" value={draft.date ?? ""}
                    onChange={(e) => setDraft({ ...draft, date: e.target.value || null })} />
                </Field>
                <Field label="Time">
                  <input type="time" className="input-pp" value={draft.time ?? ""}
                    onChange={(e) => setDraft({ ...draft, time: e.target.value || null })} />
                </Field>
                <div className="md:col-span-2">
                  <Field label="What happened">
                    <textarea className="input-pp" rows={4} value={draft.description ?? ""}
                      onChange={(e) => setDraft({ ...draft, description: e.target.value })} />
                  </Field>
                </div>
                <Field label="Location">
                  <input className="input-pp" value={draft.location ?? ""}
                    onChange={(e) => setDraft({ ...draft, location: e.target.value || null })} />
                </Field>
                <Field label="Witnesses">
                  <input className="input-pp" value={draft.witnesses ?? ""}
                ...
                <div className="md:col-span-2">
                  <Field label="How it felt (optional)">
                    <textarea className="input-pp" rows={2} value={draft.emotional_impact ?? ""}
                      onChange={(e) => setDraft({ ...draft, emotional_impact: e.target.value || null })} />
                  </Field>
                </div>
              </div>
            )}
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
              <button onClick={closeReview} className="btn-ghost">Cancel</button>
              {!reviewBusy && !draft && !reviewError && reviewFor && (
                <button onClick={() => runReview(reviewFor)} className="btn-primary inline-flex items-center gap-1">
                  <Sparkles size={13} /> Run AI review
                </button>
              )}
              {draft && (
                <button onClick={saveDraftAsIncident} disabled={savingIncident}
