...
function ShareWithAttorney() {
  ...
  const [cases, setCases] = useState<
    Array<{
      ...
      highlighted_incident_ids: string[] | null;
      attached_evidence_ids: string[] | null;
    }>
  >([]);
  ...
  useEffect(() => {
    ...
    supabase
      .from("cases")
      .select("id,case_name,other_party,highlighted_incident_ids,attached_evidence_ids")
      .eq("user_id", user.id)
  ...
  return (
    <div>
      ...
      {justCreated && (
        <div className="card-pp mt-6" style={{ borderLeft: "3px solid var(--safe)" }}>
          <div className="flex items-center gap-2">
            <Check size={16} style={{ color: "var(--safe)" }} />
            <div className="font-serif text-[18px]">Secure access link generated</div>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            >
              <Mail size={14} /> Email to attorney
            </a>
            <button onClick={() => setJustCreated(null)} className="btn-ghost">
              Done
            </button>
          </div>
          <p className="mt-3 text-[12px]" style={{ color: "var(--muted-foreground)" }}>
            This link expires in {days} days. Your attorney cannot share or download your data
          ...
          <div className="card-pp space-y-4">
                  ...
                  >
                    ...
                    {cases.map((c) => (
                      <option key={c.id} value={c.id}>
                        {c.case_name?.trim() || c.other_party?.trim() || "Untitled case"}
                      </option>
                    ))}
                  </select>
                  {(() => {
                    ...
                    if (!sel) return null;
                    const empty =
                      (sel.highlighted_incident_ids ?? []).length === 0 &&
                      (sel.attached_evidence_ids ?? []).length === 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        <Link to="/case-builder" style={{ textDecoration: "underline" }}>
                          Add them in Case Builder
                        </Link>{" "}
                        first, or share all cases instead.
                      </div>
                    );
                  })()}
                </div>
            ...
            <div>
              <label className="label-eyebrow">
                Or share a specific date range only (optional)
              </label>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Add a note for your attorney"
              />
            </div>
      ...
      <div className="mt-3 space-y-2">
        {!data ? (
          <div className="card-pp">Loading…</div>
        ) : data.links.filter((l) => l.status === "active").length === 0 ? (
          <div className="card-pp text-[13px]" style={{ color: "var(--muted-foreground)" }}>
            No attorneys connected yet.
          </div>
        ) : (
          data.links
            ...
            .map((l) => (
              <div
                key={l.id}
                className="card-pp flex items-start justify-between gap-3"
                style={{ borderLeft: "3px solid var(--safe)" }}
              >
                <div style={{ flex: 1 }}>
                  <div className="flex items-center gap-2">
                    <ShieldCheck size={14} style={{ color: "var(--safe)" }} />
                    <div className="font-serif text-[16px]">
                      {l.profile?.full_name ?? "Attorney"}
                    </div>
                  </div>
                  <div className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
                    {l.profile?.firm_name && `${l.profile.firm_name} · `}
                    {l.profile?.email}
                  </div>
                  ...
                  >
                    <input
                      ...
                      onChange={async (e) => {
                        try {
                          await setPhrasingConsent({
                            data: { link_id: l.id, consent: e.target.checked },
                          });
                          toast(
                            e.target.checked
                              ? "AI-rephrased prep enabled for this attorney."
                              : "AI-rephrased prep turned off.",
                          );
                          load();
                        } catch {
                          toast("Couldn't update. Try again in a moment.");
                        }
                      }}
                    />
                    <span>
                      Allow {l.profile?.full_name ?? "this attorney"} to see an AI-rephrased,
                      court-appropriate version of your statements for deposition prep.
                      <span
                        style={{ display: "block", color: "var(--muted-foreground)", marginTop: 2 }}
                      >
                        Your original words are never changed — this creates a separate document for
                        your attorney only. Off by default.
                      </span>
                    </span>
                  </label>
                  <label
                  ...
                  >
                    <input
                      ...
                      onChange={async (e) => {
                        try {
                          await setClioConsent({
                            data: { link_id: l.id, consent: e.target.checked },
                          });
                          toast(
                            e.target.checked
                              ? "Clio sharing approved."
                              : "Clio sharing turned off.",
                          );
                          load();
                        } catch {
                          toast("We couldn't update that setting. Try again in a moment.");
                        }
                      }}
                    />
                    <span>
                      ...
                      <span
                        style={{ display: "block", color: "var(--muted-foreground)", marginTop: 2 }}
                      >
                        Turned off. Clio has not approved our application, so no case reference can
                        be shared with Clio today. Nothing about your case is sent anywhere by this
                        setting.
                      </span>
                    </span>
                  </label>
                  ...
                  <button
                    onClick={async () => {
                      const ok = await confirm({
                        title: "Revoke this attorney's access?",
                        body: "They'll lose access immediately — the link stops working and they can't open your records again. One thing to know: anything they already downloaded or printed stays on their computer. Revoking can't reach files that have already left PatternProof.",
                        confirmLabel: "Revoke",
                        cancelLabel: "Keep",
                      });
                      if (ok) {
                        await revokeLk({ data: { id: l.id } });
      ...
      <div className="mt-3 space-y-2">
        {!data ? null : data.invitations.filter((i) => i.status === "pending").length === 0 ? (
          <div className="card-pp text-[13px]" style={{ color: "var(--muted-foreground)" }}>
          ...
          data.invitations
            ...
            .map((i) => (
              <div
                key={i.id}
                className="card-pp flex flex-wrap items-start justify-between gap-3"
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    <Scale size={14} />
                    <div className="font-serif text-[16px]">
                      {i.attorney_name ?? i.attorney_email}
                    </div>
                  </div>
                  <div className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
                    {i.attorney_email}
...
function MessagePanel({ linkId, onClose }: { linkId: string; onClose: () => void }) {
  ...
  const load = useCallback(() => {
    listFn({ data: { link_id: linkId } })
      .then((r) =>
        setMessages(
          r.messages as typeof messages extends null ? never : Exclude<typeof messages, null>,
        ),
      )
      .catch(() => setMessages([]));
  }, [listFn, linkId]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{ flex: 1, overflowY: "auto", marginTop: 12, display: "grid", gap: 8, padding: 4 }}
        >
          {messages === null && (
        ...
        <div style={{ marginTop: 12, display: "grid", gap: 6 }}>
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px]" style={{ color: "var(--muted-foreground)" }}>
              ⌘/Ctrl + Enter to send · Do not use for evidence.
            </span>
            <button
