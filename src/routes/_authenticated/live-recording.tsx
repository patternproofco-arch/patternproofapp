...
function LiveRecording() {
  ...
  const [accidental, setAccidental] = useState<{ accidental: boolean; confidence: string } | null>(
    null,
  );
      ...
      {!warned && (
        ...
        >
          ...
          <button
            ...
            className="mt-3 text-[13px] font-bold underline-offset-4 hover:underline"
        ...
        {!isRecording && !item && (
          <>
            ...
            <p className="mt-1 text-[12px] opacity-70">
              Or tap the orange microphone in the corner from any screen.
            </p>
          </>
        )}
        {isRecording && (
          <>
            <button
              onClick={() => stop()}
              className="mx-auto flex h-24 w-24 items-center justify-center rounded-full pulse-rec"
              style={{ background: "var(--primary)" }}
            >
            ...
            {accidental?.accidental && accidental.confidence === "high" && (
              ...
              >
                <p className="text-[13px]">
                  This looks like it may have been accidental. Discard it?
                </p>
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
            {new Date(r.date).toLocaleDateString()} · {r.duration_seconds ?? 0}s
          </div>
        </div>
        <button onClick={del} className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
          Delete
        </button>
      </div>
      {url && <audio src={url} controls className="mt-3 w-full" />}
      {r.transcript && (
        <div className="mt-3 rounded-2xl p-3 text-[13px]" style={{ background: "var(--input)" }}>
