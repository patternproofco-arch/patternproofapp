  ...
  return (
    <div>
      ...
      <h1 className="mt-2 font-serif text-[30px]">
        Time logged on <em>your case</em>.
      </h1>
      <p className="mt-2 max-w-2xl text-[14px]" style={{ color: "var(--muted-foreground)" }}>
        A read-only view of what each attorney you've shared with has recorded — billable and
        non-billable — so you can follow along with the work being done. Reach out to your attorney
        directly if something looks off.
      </p>

      {err && (
        <div
          className="card-pp mt-6 text-[13px]"
          ...
          <div key={a.link_id} className="card-pp" style={{ borderLeft: "3px solid var(--safe)" }}>
            <div className="flex items-center gap-2">
              ...
              <div className="font-serif text-[18px]">
                {a.primary_attorney?.full_name ?? "Attorney"}
              </div>
            </div>
            <div className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
              {a.primary_attorney?.firm_name && `${a.primary_attorney.firm_name} · `}
              {a.primary_attorney?.email}
            </div>
            ...
            {a.entries.length === 0 ? (
              <div className="mt-2 text-[13px]" style={{ color: "var(--muted-foreground)" }}>
                Nothing logged yet.
              </div>
            ) : (
              <ul className="mt-2 divide-y" style={{ borderColor: "var(--border)" }}>
                {a.entries.map((e) => (
                  ...
                  >
                    ...
                    <div className="whitespace-nowrap font-serif text-[15px]">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl px-3 py-2" style={{ background: "var(--input)" }}>
