...
function CourtPacket() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const c = await supabase.from("cases").select("*").eq("user_id", user.id).order("updated_at", { ascending: false }).limit(1);
    ...
    return (
      <div>
        ...
        <div className="card-pp mt-6">
          ...
          <button onClick={() => navigate({ to: "/case-builder" })} className="btn-primary mt-4">Go to Case Builder</button>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div id="packet" style={{ background: "#fff", color: "#14131F", padding: "32px 36px", border: "1px solid rgba(20,19,31,0.14)", borderRadius: 2, fontFamily: "'IBM Plex Sans', system-ui, sans-serif", fontSize: 14, lineHeight: 1.55 }}>
        ...
        <Section title="Incident Timeline">
          {incidents.length === 0 ? <p style={{ color: "#666" }}>No incidents highlighted.</p> : (
        ...
        <Section title="Evidence Index">
          {evidence.length === 0 ? <p style={{ color: "#666" }}>No evidence attached.</p> : (
            <ol style={{ paddingLeft: 20, margin: 0 }}>
              {evidence.map((e) => {
                const linked = incidents.find((i) => i.id === e.linked_incident_id);
                return (
                  <li key={e.id} style={{ marginBottom: 6 }}>
                    <strong>{e.title}</strong> — {new Date(e.date).toLocaleDateString()} · {e.file_type}
