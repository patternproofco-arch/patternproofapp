...
export const Route = createFileRoute("/attorney/$token")({
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: "Read-only attorney access to a case bundle shared by a PatternProof user.",
      },
      { property: "og:title", content: "Shared case bundle — PatternProof" },
      {
        property: "og:description",
        content: "Private, time-limited access to a survivor's case documentation.",
      },
      { property: "og:url", content: `https://pattern-proof.tech/attorney/${params.token}` },
...
function AttorneyView() {
  ...
  useEffect(() => {
    fetcher({ data: { token } })
      .then(setBundle)
      .catch(() => setErr("We couldn't open this link."));
  }, [fetcher, token]);

  if (err)
    return (
      <Frame>
        <Message title="Link unavailable" body={err} />
      </Frame>
    );
  if (!bundle)
    return (
      <Frame>
        <Message title="Opening…" body="One moment." />
      </Frame>
    );
  if (bundle.status === "not-found")
    return (
      <Frame>
        <Message title="Link not found" body="This access link is invalid or has been deleted." />
      </Frame>
    );
  if (bundle.status === "rate-limited")
    return (
      <Frame>
        <Message
          title="Too many attempts"
          body="This link has been opened too many times in the last hour. Please wait a while and try again."
        />
      </Frame>
    );
    ...
    return (
      <Frame>
        <Message
          ...
          body="The person who shared this with you has revoked access."
        />
      </Frame>
    );
  if (bundle.status === "expired")
    return (
      <Frame>
        <Message title="Access expired" body="This link has reached its expiration date." />
      </Frame>
    );
  ...
  return (
    <Frame>
        ...
        <section className="card-pp mt-6">
          ...
          {bundle.case_overview.pattern_summary && (
            <>
              ...
              <p className="mt-1 whitespace-pre-wrap text-[14px]">
                {bundle.case_overview.pattern_summary}
              </p>
            </>
          )}
      ...
      {bundle.escalation_flags && bundle.escalation_flags.length > 0 && (
        <section className="card-pp mt-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-[20px]">Active concerns</h2>
          </div>
          <ul className="mt-3 space-y-2">
            {bundle.escalation_flags.map((f) => (
              <li key={f.id} className="text-[14px]">
                <strong>Tier {f.severity_tier}:</strong> {f.flag_type}
                {f.details ? ` — ${f.details}` : ""}
              </li>
            ))}
          </ul>
      ...
      <footer
        className="pp-card mt-10 flex items-center gap-2 text-[11px]"
...
function ThumbCard({ ev }: { ev: NonNullable<Bundle["evidence"]>[number] }) {
  return (
    <div className="card-pp" style={{ padding: 12 }}>
      ...
      {ev.file_type === "audio" && ev.signed_url && (
        <audio controls src={ev.signed_url} className="mt-2 w-full" />
      )}
      {ev.file_type === "video" && ev.signed_url && (
        <video controls src={ev.signed_url} className="mt-2 max-h-40 w-full rounded-lg" />
      )}
