...
function SafetyPage() {
  ...
  <TrustPage title="Survivor Safety" subtitle="What PatternProof does — and does not do — to reduce risk.">
    ...
    <Section title="Shared devices and accounts">
      <ul>
        <li>Use a device the other person cannot unlock. If that is not possible, sign out fully when you finish and clear recent files.</li>
        <li>Use a private browsing window when you cannot control history.</li>
        <li>Set your notifications to generic text. PatternProof will not send SMS by default and does not use sensitive email subjects.</li>
        <li>Consider a separate email address the other person does not know about.</li>
      </ul>
    </Section>
    <Section title="Notifications and previews">
      <p>App-switcher previews and lock-screen notifications can reveal content. Where your operating system supports it, hide previews for the browser and email apps you use with PatternProof.</p>
    </Section>
    <Section title="Account recovery">
      <p>Use a recovery method the other person does not control — a private email address, an authenticator app, or a passkey. Do not rely on SMS recovery to a phone number they may access.</p>
    </Section>
