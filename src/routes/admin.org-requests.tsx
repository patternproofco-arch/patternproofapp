...
export const Route = createFileRoute("/admin/org-requests")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Internal review queue for DV organization access requests." },
      { property: "og:title", content: "Org access requests — PatternProof admin" },
      { property: "og:description", content: "Internal review queue for DV organization access requests." },
...
type Req = {
  id: string; org_name: string; contact_name: string; email: string;
  contact_role: string | null; survivors_per_month: string | null; message: string | null;
...
function AdminOrgRequests() {
  ...
  const [links, setLinks] = useState<Array<{ code: string; request_id: string | null; is_active: boolean }>>([]);
  ...
  const load = useCallback(() => {
    listFn()
      .then((r) => { setRows(r.requests as Req[]); setLinks(r.links); })
  ...
  const deny = async (id: string) => {
    setBusy(id);
    try { await denyFn({ data: { request_id: id } }); load(); }
    catch { toast.error("We couldn't update that request."); }
    finally { setBusy(null); }
  };
  ...
  return (
    <Wrap>
      ...
      {rows.length === 0 && <p style={{ fontSize: 14, color: "var(--muted-foreground)" }}>Nothing waiting right now.</p>}
      <div style={{ display: "grid", gap: 12 }}>
        {rows.map((r) => {
          const link = links.find((l) => l.request_id === r.id);
          return (
            <div key={r.id} style={{ border: "1px solid var(--border)", borderRadius: 2, padding: 16, background: "#FFFFFF" }}>
              <div style={{ fontWeight: 700 }}>{r.org_name}</div>
              <div style={{ fontSize: 13, color: "var(--muted-foreground)" }}>
                {r.contact_name} · {r.email}{r.contact_role ? ` · ${r.contact_role}` : ""}
              ...
              <div style={{ fontSize: 12, color: "var(--muted-foreground)", marginTop: 8 }}>
                {r.status}{link ? ` · code ${link.code}${link.is_active ? "" : " (off)"}` : ""}
              </div>
              {r.status === "pending" && (
