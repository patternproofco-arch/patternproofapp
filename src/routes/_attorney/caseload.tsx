...
export const Route = createFileRoute("/_attorney/caseload")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Aggregate view of clients needing attention across your active caseload.",
      },
...
function CaseloadPage() {
  ...
  useEffect(() => {
    fetcher()
      .then(setData)
      .catch((e) => setErr(e?.message ?? "Could not load caseload."));
    conflictFetcher()
      .then(setConflicts)
  ...
  if (!data)
    return (
      <div style={{ padding: 16, fontSize: 13, color: "var(--att-text-2)" }}>Loading caseload…</div>
    );
    ...
    <div style={{ display: "grid", gap: 20 }}>
      ...
      >
        ...
        <Kpi
          ...
          label="Unconfirmed AI drafts"
          value={totals.total_unconfirmed_ai_drafts}
          tone={totals.total_unconfirmed_ai_drafts ? "warn" : undefined}
        />
        <Kpi
          icon={<Clock size={14} />}
          label="Disengaged (30d)"
          value={totals.disengaged_client_count}
          tone={totals.disengaged_client_count ? "warn" : undefined}
        />
        <Kpi
          icon={<AlertTriangle size={14} />}
          label="Clients w/ unreviewed severity"
          value={totals.clients_with_unreviewed_severity_indicators}
          tone={totals.clients_with_unreviewed_severity_indicators ? "warn" : undefined}
        />
      </div>
      ...
      <div style={{ boxShadow: "var(--pp-shadow-sm)", borderRadius: 16, overflow: "hidden" }}>
        <div
          style={{
            ...
            gap: 8,
            padding: "10px 14px",
            background: "var(--att-surface-2, #f7f7f5)",
            fontSize: 11,
            textTransform: "uppercase",
            letterSpacing: 0.6,
        ...
        {clients.length === 0 && (
          <div style={{ padding: 20, fontSize: 13, color: "var(--att-text-2)" }}>
            No active clients yet.
          </div>
        )}
        {clients.map((c) => {
          const attention =
            c.unconfirmed_ai_draft_count > 0 ||
            c.disengaged_30d ||
            c.unreviewed_severity_indicator_count > 0;
          return (
            <Link
              key={c.link_id}
              to="/clients/$clientId"
            ...
            >
              ...
              <div style={{ color: "var(--att-text-2)" }}>
              ...
              <Chip
                value={c.unreviewed_severity_indicator_count}
                warn={c.unreviewed_severity_indicator_count > 0}
              />
      ...
      <p style={{ fontSize: 11, color: "var(--att-text-2)", maxWidth: 640 }}>
        Documentation activity is a workflow signal, not a legal or clinical assessment.
        "Disengaged" means no incidents or evidence were added by the survivor in the last 30 days —
        it does not imply anything about their situation.
      </p>
    </div>
  );
}
...
function Kpi({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: "var(--font-sans)", fontSize: 26, marginTop: 6 }}>{value}</div>
    </div>
  );
}
