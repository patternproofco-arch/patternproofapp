...
export const Route = createFileRoute("/org-feedback")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tell us how PatternProof is landing with survivors your organization refers." },
      { property: "og:title", content: "DV organization feedback — PatternProof" },
      { property: "og:description", content: "Feedback from DV organizations who refer survivors to PatternProof." },
          ...
          {done ? (
            <div>
              ...
              <p style={{ color: "#2C3826", lineHeight: 1.6 }}>
                Advocate feedback shapes what we build next. If you left contact info we'll follow up personally.
              </p>
            </div>
          ) : (
            <>
              <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-0.02em", margin: "14px 0 12px", color: "#1F2D1A" }}>
                How is PatternProof landing with your survivors?
              </h1>
              <p style={{ color: "#2C3826", lineHeight: 1.65, marginBottom: 24 }}>
                A few short questions. Skip anything you'd rather not answer. Your organization name and contact info are optional — leave them blank to respond anonymously.
              </p>
              ...
              <form onSubmit={submit} style={{ display: "grid", gap: 20 }}>
                ...
                <OField label="How easy was it to refer a survivor to PatternProof?">
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {REFERRAL_EASE.map((o) => (
                      <button key={o.value} type="button" onClick={() => setEase(o.value)} style={pillBtn(ease === o.value)}>
                        {o.label}
                      </button>
                    ))}
                  </div>
                ...
                {error && <div style={{ color: "#8B3A3A", fontSize: 13, fontWeight: 600 }}>{error}</div>}
...
function OField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 13, fontWeight: 700, color: "#1F2D1A", marginBottom: 8, lineHeight: 1.45 }}>{label}</label>
      {children}
    </div>
  );
}
function OInput({ value, onChange, type = "text" }: { value: string; onChange: (v: string) => void; type?: string }) {
  return <input type={type} value={value} onChange={(e) => onChange(e.target.value)} maxLength={300} style={inputStyle} />;
}
function OTextArea({ value, onChange, rows }: { value: string; onChange: (v: string) => void; rows: number }) {
  return <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={rows} maxLength={2000} style={{ ...inputStyle, resize: "vertical", fontFamily: "inherit" }} />;
}
