...
export const Route = createFileRoute("/sample-case")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "A fictional, anonymized sample of what an attorney receives when a client shares her PatternProof record — structured chronology, source-linked exhibits, cross-reference." },
      { property: "og:title", content: "Sample case — PatternProof" },
      { property: "og:description", content: "A fictional, anonymized sample of an attorney's view of a shared client record." },
      { property: "og:url", content: "https://pattern-proof.tech/sample-case" },
      { property: "og:type", content: "website" },
      { name: "twitter:title", content: "Sample case — PatternProof" },
      { name: "twitter:description", content: "A fictional, anonymized sample of an attorney's view." },
    ],
...
const EXHIBITS: Exhibit[] = [
  ...
  },
  {
    ...
    detail: "iMessage export, SHA-256 preserved. Language escalates from grievance → name-calling → explicit threat at message 41.",
  },
  {
    id: "015",
    ...
    detail: "Survivor recalls the week before spring break. Same pattern as Feb 03. No photo; text to sister on Apr 09 references 'again today'.",
  },
  {
    id: "016",
    date: "Date unknown",
    ...
    detail: "Bank statement shows access-revocation flag. Survivor cannot pinpoint the day; discovered on a routine login attempt.",
  },
  {
    id: "018",
    ...
    detail: "Audio file with AI-generated transcript (labeled unverified). Speaker 1 identified as opposing party by survivor.",
  },
  {
    id: "019",
    ...
    detail: "Screenshot of Life360 notification. GPS metadata quarantined server-side; not surfaced to attorney view.",
  },
  {
    id: "022",
...
const CLUSTERS: Cluster[] = [
  ...
  },
  {
    ...
    summary: "Survivor's journal locates the call at home; carrier metadata places originating tower ~4 miles away.",
    items: [
...
function SampleCase() {
  return (
    <div style={{ background: PAPER, color: INK, minHeight: "100vh", fontFamily: SANS }}>
      ...
      <section style={{ maxWidth: 900, margin: "0 auto", padding: "clamp(48px,8vw,88px) 24px 24px" }}>
        ...
        <h1 style={{ fontFamily: SERIF, fontWeight: 300, fontSize: "clamp(2rem,4.8vw,3.2rem)", lineHeight: 1.08, letterSpacing: "-0.02em", margin: 0 }}>
          Client: <em>Jane R.</em> <span style={{ color: MUTED }}>(fictional)</span>
        </h1>
        <p style={{ marginTop: 16, fontSize: 15.5, lineHeight: 1.6, color: "#3A3849", maxWidth: 680 }}>
          This is a fictional sample case, shown to illustrate what an attorney receives — no real survivor's data.
        </p>
        <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 10, maxWidth: 680 }}>
      ...
      <section style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 40px" }}>
        ...
        <p style={{ fontFamily: SERIF, fontSize: 16, lineHeight: 1.6, color: MUTED, margin: "0 0 18px", maxWidth: 640 }}>
          Detected deterministically from the record. Surfaced so you can reconcile them with the client before opposing counsel does.
        </p>
        <div style={{ display: "grid", gap: 12 }}>
          {CLUSTERS.map((c) => (
      ...
      <section style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 56px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          <button
            ...
            style={{
              background: "#CBCAD3", color: "#7A798A",
      ...
      <section style={{ borderTop: `1px solid ${RULE}`, background: "#EFEDE6" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px" }}>
          <div style={{ fontFamily: SERIF, fontWeight: 300, fontSize: "clamp(1.4rem,2.6vw,1.8rem)", lineHeight: 1.3, color: INK, maxWidth: 620 }}>
            Ready to see this with your own client's record?
          </div>
          <Link
            to="/lawyer-signup"
            style={{
              display: "inline-block", marginTop: 22, background: NAVY, color: "#F4F6FB",
              padding: "14px 26px", fontFamily: MONO, fontSize: 13, letterSpacing: "0.1em",
  ...
  return (
    <header style={{ borderBottom: `1px solid ${RULE}` }}>
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "18px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Link to="/for-attorneys" style={{ fontFamily: MONO, fontSize: 12, letterSpacing: "0.14em", color: INK, textDecoration: "none", textTransform: "uppercase" }}>
          ← For attorneys
        </Link>
...
function MetaCell({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ border: `1px solid ${RULE}`, padding: "10px 12px", background: PAPER }}>
      <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.14em", color: MUTED, textTransform: "uppercase" }}>{label}</div>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "baseline", justifyContent: "space-between" }}>
        <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", color: INK }}>
          Exhibit {ex.id} · {ex.date}
        </div>
        <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.14em", color: certColor, textTransform: "uppercase" }}>
          {certLabel}
        </div>
      </div>
...
function ClusterRow({ c }: { c: Cluster }) {
  return (
    ...
    >
      <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", color: NAVY }}>
        {c.label}
      </div>
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: "10px 0 0", display: "grid", gap: 6 }}>
        {c.items.map((i) => (
          <li key={i} style={{ display: "grid", gridTemplateColumns: "16px 1fr", gap: 8, fontSize: 13.5, lineHeight: 1.55, color: INK }}>
