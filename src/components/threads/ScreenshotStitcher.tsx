...
export function ScreenshotStitcher({ onDone, onCancel }: Props) {
  ...
  return (
    <div style={{ borderRadius: 20, padding: 22, background: "rgba(255,255,255,0.8)", border: "1px solid rgba(124,92,196,0.2)" }}>
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="label-eyebrow" style={{ color: "#7C5CC4" }}>Tier 1 · Fastest</div>
          <h3 style={{ fontFamily: '"Instrument Serif", serif', fontSize: 24, color: "#1A1714", marginTop: 4 }}>Add screenshots of the thread</h3>
        </div>
      ...
      <button type="button" onClick={() => inp.current?.click()}
        style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 14px", borderRadius: 999,
          background: "linear-gradient(90deg,#7C5CC4,#2F8D85)", color: "#fff", fontWeight: 700, fontSize: 13 }}>
      ...
      {shots.length > 0 && (
        <div className="mt-4 grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))" }}>
          {shots.map((s, idx) => (
            <div key={s.id} style={{ position: "relative", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(124,92,196,0.2)" }}>
      ...
      <div className="mt-5 flex items-center justify-end gap-2">
        <button type="button" onClick={save} disabled={busy || shots.length === 0}
          style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 16px", borderRadius: 999,
            background: shots.length === 0 ? "#C9C0BC" : "linear-gradient(90deg,#7C5CC4,#2F8D85)",
