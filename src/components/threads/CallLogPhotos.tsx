...
export function CallLogPhotos({ onDone, onCancel }: Props) {
  ...
  return (
    <div style={{ borderRadius: 0, padding: 22, background: "#FAF8F4", border: "1px solid rgba(122,31,61,0.22)" }}>
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="label-eyebrow" style={{ color: "#4132B4" }}>Call log · Photos</div>
          <h3 style={{ fontFamily: "'Fraunces', Georgia, serif", fontWeight: 300, fontSize: 24, color: "#1A1224", marginTop: 4 }}>Import your call history</h3>
        </div>
      ...
      <div className="grid gap-3 mb-3" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
        <div>
          ...
          <select value={platform} onChange={(e) => setPlatform(e.target.value as typeof platform)} className="input-pp mt-1">
        ...
        <div>
          ...
          <input value={notes} onChange={(e) => setNotes(e.target.value)} className="input-pp mt-1" placeholder="e.g. week before the hearing" />
        </div>
      </div>
          ...
          {shots.map((s, idx) => (
            <div key={s.id} style={{ position: "relative", borderRadius: 0, overflow: "hidden", border: "1px solid rgba(122,31,61,0.2)" }}>
              ...
              <button type="button" onClick={() => remove(idx)} aria-label="Remove"
                style={{ position: "absolute", top: 4, right: 4, background: "rgba(255,255,255,0.9)", borderRadius: 2, padding: 4 }}>
                <X size={12} />
              </button>
            </div>
          ))}
