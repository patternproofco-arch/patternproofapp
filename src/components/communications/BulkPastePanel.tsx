...
function parseThread(raw: string, fallbackDate: string): ParsedRow[] {
  ...
  const re2 =
    /^(\d{1,2})\/(\d{1,2})\/(\d{2,4}),?\s+(\d{1,2}:\d{2})(?::\d{2})?\s*(AM|PM|am|pm)?\s*[-–—]\s*([^:]{1,80}):\s*(.+)$/;
  // Name: message
  const re3 = /^([^:]{1,80}):\s*(.+)$/;
...
export function BulkPastePanel({ userId, onSaved, onClose }: Props) {
  ...
  const parse = () => {
    ...
    if (!parsed.length) {
      toast("No messages found. Check the format and try again.");
      return;
    }
    setRows(parsed);
  };
  ...
  const save = async () => {
    ...
    setBusy(false);
    if (error) {
      toast("We couldn't save the thread. Try again in a moment.");
      return;
    }
  ...
  return (
    <div className="card-pp space-y-3">
      ...
      <p className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
        Paste a text thread. We'll parse WhatsApp-style lines, <code>[YYYY-MM-DD HH:MM] Name:</code>
        , or plain <code>Name: message</code>.
      </p>
      ...
      <div className="flex gap-2">
        <button type="button" onClick={parse} className="btn-ghost">
          Parse
        </button>
        {rows.length > 0 && (
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="text-[11px]" style={{ color: "var(--muted-foreground)" }}>
                  {r.date}
                  {r.time ? ` · ${r.time}` : ""} · {r.from_party} · {direction(r.from_party)}
                </div>
                <div className="mt-1 whitespace-pre-wrap text-[13px]">{r.content}</div>
              </div>
              <button
                type="button"
                aria-label="Toggle harassment flag"
                onClick={() =>
                  setRows(
                    rows.map((x, i) =>
                      i === idx ? { ...x, harassment_flag: !x.harassment_flag } : x,
                    ),
                  )
                }
                className="rounded-md p-1"
                style={{ color: r.harassment_flag ? "var(--primary)" : "var(--muted-foreground)" }}
              >
