...
export function CommForm({ userId, incidents, onSaved }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!parsed.success) { toast(parsed.error.issues[0].message); return; }
    ...
    if (screenshot) {
      ...
      if (up.error) { setBusy(false); toast(`We couldn't save "${screenshot.name}". Try again in a moment.`); return; }
      screenshotPath = key;
    }
    ...
    setForm({ date: todayLocal(), time: "", channel: "text", direction: "incoming", from_party: "", content: "", harassment_flag: false, notes: "", linked_incident_id: "" });
    setScreenshot(null);
    onSaved();
  };
  ...
  return (
    <form onSubmit={submit} className="card-pp space-y-3">
      ...
      <div>
        ...
        <select value={form.linked_incident_id} onChange={(e) => setForm({ ...form, linked_incident_id: e.target.value })} className="input-pp mt-1">
