        ...
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <span className="label-eyebrow">
              {formatDateLocal(comm.date)}
              {comm.time && ` · ${comm.time.slice(0, 5)}`}
            </span>
            <span
            ...
            <span
              ...
              style={{ background: "var(--panel)", color: "#FFFFFF" }}
            >
              {comm.direction}
            </span>
          ...
          {comm.content && (
            <p className="mt-1 whitespace-pre-wrap text-[14px] leading-relaxed">{comm.content}</p>
          )}
          {comm.notes && (
          ...
          {screenshotUrl && (
            <img src={screenshotUrl} alt="Screenshot" className="mt-3 max-h-64 rounded-lg" />
          )}
        </div>
        <button
          onClick={() => {
            if (armDelete) onRemove();
