...
interface Props {
  incidents: Array<{
    ...
    has_escalation_flag?: boolean | null;
  }>;
  months?: number;
}
...
export function IncidentHeatMap({ incidents, months = 6 }: Props) {
  ...
  const monthCells: Array<{
    key: string;
    label: string;
    days: Array<{ date: string; day: number; cell: { count: number; flagged: boolean } | null }>;
  }> = [];
  ...
  while (cursor <= today) {
    ...
    const days: Array<{
      date: string;
      day: number;
      cell: { count: number; flagged: boolean } | null;
    }> = [];
    ...
    for (let d = 1; d <= lastDay; d++) {
      ...
      days.push({
        date: iso,
        day: d,
        cell: cell ? { count: cell.count, flagged: cell.flagged } : null,
      });
    }
  ...
  const colorFor = (cell: { count: number; flagged: boolean } | null) => {
    ...
    if (cell.flagged)
      return `color-mix(in oklab, var(--primary) ${Math.round(intensity * 100)}%, var(--input))`;
  ...
  return (
    <div>
              ...
              {mo.days.map((d, i) =>
                d.day === 0 ? (
                  ...
                  <Link
                    ...
                    style={{
                      background: colorFor(d.cell),
                      color: d.cell ? "var(--sidebar-active)" : "var(--muted-foreground)",
                      paddingTop: 4,
                    }}
                  >
                    {d.day}
      ...
      >
        ...
        <div className="flex gap-1">
          {[0, 1, 2, 3, 4].map((n) => (
            <div
              ...
              style={{
                background: `color-mix(in oklab, var(--accent) ${20 + n * 20}%, var(--input))`,
              }}
            />
          ))}
        </div>
        ...
        <div
          className="ml-3 h-3 w-3 rounded"
          style={{ background: "color-mix(in oklab, var(--primary) 70%, var(--input))" }}
        />
        <span>Flagged</span>
      </div>
    </div>
