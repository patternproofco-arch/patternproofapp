...
export function AddFromJournalModal({
  open,
  onClose,
  onSaved,
}: {
  ...
  const handleUpload = async (f: File | null) => {
    ...
    try {
      ...
      const r = await ocr({
        data: { signedUrl: signed.data.signedUrl, mimeType: f.type || "image/jpeg" },
      });
      if (!r.ok) {
        toast(
          "We couldn't read the writing on that page. Try a clearer photo, or type it in below.",
        );
        setText("");
        setStep("review-text");
        return;
  ...
  const toggleType = (i: number, t: string) => {
    setDrafts((d) =>
      d.map((x, idx) => {
        ...
        return {
          ...x,
          abuse_types: x.abuse_types.includes(t)
            ? x.abuse_types.filter((y) => y !== t)
            : [...x.abuse_types, t],
        };
      }),
    );
  ...
  const saveAll = async () => {
    ...
    if (!selected.length) {
      toast("Pick at least one incident to save.");
      return;
    }
    setStep("saving");
    ...
    toast(
      `Saved ${rows.length} ${rows.length === 1 ? "record" : "records"}. Your entries are safe.`,
    );
    onSaved();
    close();
  };
        ...
        >
          <div className="flex items-center gap-2">
            {step !== "upload" && step !== "saving" && (
              <button
                onClick={() => setStep(step === "pick" ? "review-text" : "upload")}
          ...
          <button onClick={close} aria-label="Close" className="rounded-lg p-1.5 hover:bg-black/5">
            <X size={16} />
          </button>
        </div>
          ...
          {step === "upload" && (
            <div className="space-y-4">
              <p className="text-[14px]" style={{ color: "var(--foreground)" }}>
                Take a photo of your handwritten journal page or typed notes. We'll read it and help
                you turn it into clean, separate incident records.
              </p>
              <p className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
                Bad quality? Take another. We just need to read it.
              </p>
              <label
                className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed p-8 text-center transition-colors hover:bg-black/5"
                style={{ borderColor: "var(--border)" }}
              >
                {busy ? (
                ...
                <span className="text-[14px] font-semibold">
                  {busy ? "Reading your page…" : "Upload a photo of your page"}
                </span>
          ...
          {step === "review-text" && (
            <div className="space-y-3">
              <div>
                ...
                <p className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
                  Edit anything that's wrong — typos, missing words, unclear spots.
                </p>
              </div>
              <textarea
                value={text}
            ...
            <div className="space-y-4">
              <div>
                <h3 className="font-serif text-[16px]">
                  We found {drafts.length} {drafts.length === 1 ? "incident" : "incidents"}
                </h3>
                <p className="text-[12px]" style={{ color: "var(--muted-foreground)" }}>
                  Check the ones you want to save. Edit any details before saving.
                </p>
              </div>
              <div className="space-y-3">
                {drafts.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      background: d.selected ? "var(--card)" : "transparent",
