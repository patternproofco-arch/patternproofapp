...
export function ThreadFilters({
  search, onSearch, from, to, onFrom, onTo, contacts, contactId, onContact, count,
}: Props) {
  return (
    <div
    ...
    >
      ...
      <input
        type="date" className="input-pp" style={{ minHeight: 44 }}
        value={from} onChange={(e) => onFrom(e.target.value)} aria-label="From date"
      />
      <div className="flex items-center gap-2">
        <input
          type="date" className="input-pp" style={{ minHeight: 44 }}
          value={to} onChange={(e) => onTo(e.target.value)} aria-label="To date"
        />
