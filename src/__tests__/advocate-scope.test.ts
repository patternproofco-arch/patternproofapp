...
const fullIncident = {
  id: "i1", user_id: "u1", date: "2026-01-02", time: "18:00",
  location: "Home", description: "What happened, in her words.",
  abuse_types: ["emotional"], witnesses: "Neighbor Jane", emotional_impact: "Frightened",
  severity_level: 4, has_escalation_flag: true, source: "survivor", confidence: 0.9,
  date_precision: "exact", date_range_start: null, date_range_end: null,
};

const fullEvidence = {
  id: "e1", user_id: "u1", title: "Text screenshot", date: "2026-01-02",
  description: "Message thread", file_type: "image", file_url: "https://x/y.png",
  storage_path: "u1/y.png", gps_lat: 40.1, gps_lon: -74.2, gps_reveal_opt_in: true,
  raw_metadata: { GPSLatitude: 40.1 }, original_filename: "IMG_0042.HEIC",
  sha256: "abc", transcript: "full audio transcript", transcript_segments: [{ t: 0 }],
  voice_caption: "her voice note", voice_caption_audio_url: "https://x/a.m4a",
  linked_incident_id: "i1", date_precision: "exact", date_range_start: null, date_range_end: null,
};
...
describe("advocate scope projection", () => {
  it("gives advocates the survivor-approved incident narrative only", () => {
    ...
    expect(Object.keys(out).sort()).toEqual(
      ["abuse_types", "date", "date_precision", "date_range_end", "date_range_start", "description", "id", "location", "time"],
    );
  });
  ...
  it("enforces the granted date window", () => {
    const rows = [{ date: "2025-12-01" }, { date: "2026-01-02" }, { date: "2026-03-01" }, { date: null }];
