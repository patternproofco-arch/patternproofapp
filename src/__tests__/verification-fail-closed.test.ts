...
describe("verification lookup failure", () => {
  ...
  it("uses exact day-180 fail-closed, and rejects future timestamps", () => {
    const now = Date.parse("2026-08-31T12:00:00Z");
    // Day 180 inclusive: linked exactly 180 days ago is NOT current.
    expect(isCaseEngagementCurrent("2026-03-04T12:00:00Z", null, now)).toBe(false); // 180d
    expect(isCaseEngagementCurrent("2026-03-05T12:00:00Z", null, now)).toBe(true); // 179d
    expect(isCaseEngagementCurrent("2026-09-01T12:00:00Z", null, now)).toBe(false); // future
    expect(isCaseEngagementCurrent("2025-01-01T12:00:00Z", "2026-09-01T12:00:00Z", now)).toBe(
      false, // confirm in the future still fails closed
    );
  });
});
